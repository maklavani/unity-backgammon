...
public class TournamentWindow : GenericWindow {
	...
	public void SetImage(int user , int frame){
		users [user].transform.Find ("Image").GetComponent<Image> ().sprite = animationManager.faceImages [frames[frame] - 1];
	}
	...
	public void CheckUsernameExist () {
		...
		if (gameControl.userA != "" && usernameA == "") {
			...
			statusFrame [0] = false;
			users [0].transform.Find ("Face").GetComponent<Image> ().sprite = animationManager.faceImages [gameControl.faceA - 1];
			...
			statusFrame [1] = false;
			users [1].transform.Find ("Face").GetComponent<Image> ().sprite = animationManager.faceImages [gameControl.faceB - 1];
