...
public class StartWindow : GenericWindow {
	...
	public void ShowProfile()
	{
		if (gameControl.changeUsername == false) {
			transform.Find ("WindowUsername").gameObject.SetActive (true);
			transform.Find ("WindowProfile").gameObject.SetActive (false);
			// username.text = gameControl.username;
			// password.text = gameControl.password;
		} else {
			transform.Find ("WindowUsername").gameObject.SetActive (false);
			transform.Find ("WindowProfile").gameObject.SetActive (true);

			if(gameControl.face < 7)
				face.transform.Find ("RowA").Find (gameControl.face.ToString ()).GetComponent<Toggle> ().isOn = true;
			else
				face.transform.Find ("RowB").Find (gameControl.face.ToString ()).GetComponent<Toggle> ().isOn = true;
		}
	}
