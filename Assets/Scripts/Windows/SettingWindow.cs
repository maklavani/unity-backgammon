...
public class SettingWindow : GenericWindow {
	...
	public void UpdateInformation(){
		...
		theme.transform.Find (gameControl.theme.ToString ()).GetComponent<Toggle> ().isOn = true;
		...
		reagent.text = gameControl.reagent;
	}
	...
	public void ChangeTheme(){
		var actives = theme.ActiveToggles ();

		foreach (Toggle active in actives) {
			gameControl.theme = (Themes)(System.Enum.Parse (typeof(Themes), active.name));
			gameControl.Save ();
		}
	}
	...
	public void SetSiblings(){
		if (gameControl.language == Languages.Persian) {
			transform.Find ("Control").SetSiblingIndex (1);
			music.transform.SetSiblingIndex (0);
			sfx.transform.SetSiblingIndex (0);
			language.transform.SetSiblingIndex (0);
			theme.transform.SetSiblingIndex (0);
			reagent.transform.parent.SetSiblingIndex (0);
			reagent.transform.SetSiblingIndex (1);
			...
			theme.transform.parent.Find ("Text").GetComponent<Text> ().alignment = TextAnchor.MiddleRight;
			reagent.transform.parent.parent.Find ("Text").GetComponent<Text> ().alignment = TextAnchor.MiddleRight;
		} else {
			transform.Find ("Control").SetSiblingIndex (0);
			music.transform.SetSiblingIndex (1);
			sfx.transform.SetSiblingIndex (1);
			language.transform.SetSiblingIndex (1);
			theme.transform.SetSiblingIndex (1);
			reagent.transform.parent.SetSiblingIndex (1);
			reagent.transform.SetSiblingIndex (0);
			...
			theme.transform.parent.Find ("Text").GetComponent<Text> ().alignment = TextAnchor.MiddleLeft;
			reagent.transform.parent.parent.Find ("Text").GetComponent<Text> ().alignment = TextAnchor.MiddleLeft;
		}
	}
	...
	public void ConfirmSMS() {
		var confirm = GameObject.Find ("PopUpWindow").transform.Find ("Mobile").Find ("Confirm").GetComponent<InputField> ();
		WWWForm data = new WWWForm();
